...
def bin_rescale(
    ...
    *,
    ...
    # Aplicamos coarsen por 'range'; xarray maneja dims en cualquier posición
    coarsener = ds_trim.coarsen(range=factor, boundary="trim")
    ...
    for name, da in ds_trim.data_vars.items():
        ...
        if agg == "mean":
            ...
            rebinned = coarsener[name].mean(skipna=True)
        elif agg == "sum":
            rebinned = coarsener[name].sum()
        else:
